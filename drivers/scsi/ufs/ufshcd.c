...
ufshcd_transfer_rsp_status(struct ufs_hba *hba, struct ufshcd_lrb *lrbp)
{
	...
	if ((hba->quirks & UFSHCD_QUIRK_DUMP_DEBUG_INFO) &&
	    (host_byte(result) != DID_OK) && !hba->silence_err_logs)
					...
					int check_for_bkops)
{
	 ...
	 * turning off the link would also turn off the devicew.
	 */
...
UFS_DEV_ATTR(sense_err_count, "\"MEDIUM\":\"%u\",\"HWERR\":\"%u\"\n",
...
static void ufshcd_add_vendor_sysfs_node(struct ufs_hba *hba)
{
	int err  = -ENOMEM;
	...
	if (err)
		dev_info(hba->dev, "cannot create sysfs group err: %d\n", err);
}
...
static inline void ufshcd_add_sysfs_nodes(struct ufs_hba *hba)
{
	...
	ufshcd_add_vendor_sysfs_node(hba);
}
